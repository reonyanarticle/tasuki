# tasuki(襷)

AI エージェント間の襷(タスクと契約)の受け渡しを中継所(ゲート)で検めながら、GitHub issue を自走で完走させる Claude Code plugin。

名前は駅伝の襷に由来する。
構成要素は次の対応で読み替えられる。

| 駅伝 | tasuki |
|---|---|
| 区間 | フェーズ |
| 走者 | agent |
| 中継所 | ゲート |
| 襷 | 契約 |
| 繰り上げスタート | 打ち切り条件 |
| 往路と復路 | 降りるゲートと昇るゲート |

## なぜゲートか

仕事の受け渡しでは、「この出力は、受け手が追加の解釈なしに受け取れる抽象度か」というズレがつなぎ目ごとに生じる。
抽象度に絶対の基準はない。
適切な粒度は、受け手が誰か(次の分割役か、実装役か、人間か)で変わる。
AI が実装からレビューまでを自律で回すループでは、人間がこのズレを毎回は是正できない。
そこで tasuki は、ズレ検知を仕組みとして各つなぎ目に埋め込む。
これがゲートである。
判定は絶対基準ではなく、受け手が契約で宣言した「待ち位置」との照合になる。
思想の全体は [docs/PHILOSOPHY.md](docs/PHILOSOPHY.md) にある。

## 何をするか

GitHub issue 駆動の自律ループ(実装と検証)の各フェーズのつなぎ目に、抽象度ゲートを置く。
ゲートは出力を契約の待ち位置と照合し、3値で判定する。

| 判定 | 意味 |
|---|---|
| `PASS` | 引き渡し位置が待ち位置と一致している |
| `TOO_ABSTRACT` | 曖昧すぎて、受け手が追加の解釈なしには受け取れない |
| `TOO_CONCRETE` | 詳しすぎて、受け手の判断余地を奪っている |

検証(実験)と開発の両方で使える汎用構成とし、まず Python を対象に実装している。

## 使い方

1. plugin を導入し、対象リポジトリで `/tasuki:loop-init` を実行する。契約プロファイル、issue と PR のテンプレート、CI workflow、ラベルが生成される
2. 運用開始前に、判定例の fixture を5件程度手書きする。手順は `tasuki:baton-contract` skill が案内する
3. 親 issue の配下に子 issue を起票し、`/tasuki:loop <親 issue 番号>` で自走させる
4. `/tasuki:loop-status` で、人間の裁定待ち(triage inbox)と進行状況を確認する。**マージは常に人間が実行する**

## ドキュメント

設計文書は [docs/](docs/README.md) にまとまっている。
読む順序と索引は [docs/README.md](docs/README.md) が案内する。

## ステータス

段階導入のフェーズ2まで完了している(有効なゲートは G2 着手、GM 形式、G3 成果の3つ)。
フェーズ3(G0 受理、G1 分割、G4 統合とレイヤー並列実行)は未着手である。
各フェーズの受け入れ条件と E2E の実施結果は [docs/ROADMAP.md](docs/ROADMAP.md) に記録している。

## License

[MIT](LICENSE)
