...
このコメントには **`tasuki:plan-diagram` skill の手順で、依存とレイヤーと状態を示す mermaid の計画図(flowchart)を含める**(人間が全体設計を俯瞰するため)。各子の状態は `gate:*` / `loop:*` ラベルから決める。以後の run では同じ計画コメントを編集して状態色を更新する(冪等)。
