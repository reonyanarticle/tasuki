...
worker を起動するたびに WIP(観点 #24)を再確認し、上限到達中の子は「WIP 待ち」として保留する。WIP 待ちは**終端状態ではない停止状態**である。同一 run 内で人間のマージを待たず、run を終了して報告する(次の run で解放されていれば着手する)。解放条件が親 PR の人間マージであることを報告に明示する。
...
最終レイヤーまで完了し、**全子 issue が決着(統合ブランチへの取り込み、または人間による不採用クローズ)したら**判定する。両者はどちらも closed になるため、**取り込みの完了コメントの有無で区別する**(完了コメントが無い closed は不採用として扱う)。不採用クローズされた子の親要件は「未充足」として統合ゲートに渡し、要件の縮小(親本文の修正)か追加分割かを人間に問う材料にする。
...
**3c に入るとき、orchestrator は次の2つを先に行う。**
...
1. **親 PR の本文を承認者向けに最終化する。** 親 PR は監査の記録ではなく**承認の道具**である。本文は2段で構成する。
   ...
   **承認のための材料は、本文の編集ではなく「新規コメント」として投稿する**(3c 入場時、親 PR の check-runs 全緑を確認してから)。人間はタイムラインを下へ読むため、最後に現れるコメントが「いま読むべきもの」の自然な置き場である。本文の編集は静かに書き換わるだけで、読み手に届かない。本文側には大観と「承認材料は最後のコメント」への一言だけを残す。
   検証が通る前に承認の文言を出すと、赤のまま「承認してほしい」と読める文章が世に出る。順序は、機械の検証が先、人間への依頼が後である。
   このコメントは1件だけとし、差し戻し後の再入では同じコメントを編集して更新する(冪等)。

   - **検証の要約**:check-runs 全緑と統合ゲート PASS を1行で(承認の前提が揃った、という宣言)
   ...
   - **やらなかったこと**:スコープ外に残した既知の課題。**どの子の作業で見つかったかを添える**(次の親 issue の種として辿れるように)
...
**これは人間が起動するコマンドである。** orchestrator は自分でレビューを起動せず、実行を促して結果を待つ(課金と実行時間が人間の判断に属するため)。
...
5. 問題が無ければそのまま人間のマージ待ちとする。**マージは人間が行う**(このマージが default branch への唯一の反映点。子 issue は取り込み時に close 済みで、`Closes` 列挙は close し損ねの保険)。親 issue の close も人間が行う(完了の定義への最終適合は人間の判断)
