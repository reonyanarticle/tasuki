...
1. **親 PR の本文を承認者向けに最終化する。** 親 PR は監査の記録ではなく**承認の道具**である。コードを読まなくても「何が変わり、何を承認するのか」が分かるように、契約の `parent_pr_required_fields` の欄を埋める。
   - **何が変わるか**:利用者に見える変化と、内部だけの変化を分けて書く
   - **承認してほしい判断**:**各子の実装方針コメントの「選択と理由」から、親 issue の要件に書かれていない裁量の決定を集めて列挙する**(例:直列化の表現、エラー時の終了コード、退けた代替案)。承認とはこの判断への同意であり、ここが親 PR の本体である
   - **やらなかったこと**:スコープ外に残した既知の課題(次の親 issue の種)
   - **リスクと戻し方**:壊れたときに何が起き、どう戻すか
   - 受け入れ条件の充足表は要点だけ残し、詳細は子へのリンクにする
