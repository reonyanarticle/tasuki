...
1. **親 PR の本文を承認者向けに最終化する。** 親 PR は監査の記録ではなく**承認の道具**である。本文は2段で構成する。
   ...
   **承認のための材料は、本文の編集ではなく「新規コメント」として投稿する**(レビューが所見なし、または所見の修正を取り込み終えてから)。人間はタイムラインを下へ読むため、最後に現れるコメントが「いま読むべきもの」の自然な置き場である。本文の編集は静かに書き換わるだけで、読み手に届かない。本文側には大観と「承認材料は最後のコメント」への一言だけを残す。
   ...
   - **承認してほしい判断**:各子の実装方針コメントの「選択と理由」から、親 issue の要件に書かれていない裁量の決定を集めて列挙する(**maker が実装方針コメントを持たない契約では、その maker の第1手コメントを入力元にする**。research の researcher なら調査戦略コメントの「採用と除外の基準」と「反証の探し方」、および調査文書の「除外と不採用の記録」から、親の問いに書かれていない判断を集める)。**各判断に「どの子(#N)で決めたか」と「どこで検められたか」(検めたゲートや検証したテスト)を必ず添える**。読み手が「この判断は誰がいつ下し、何が担保しているか」を1行で辿れることが、コードを読まずに承認できる条件である
