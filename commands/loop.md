...
このコメントは **`tasuki:plan-diagram` skill の4部構成**にする:進捗サマリ、mermaid の計画図(flowchart。ノードに issue 番号・タイトル・ゲート状況・PR・予算を載せ、依存の辺とレイヤーの subgraph と凡例を含む)、要件カバレッジ図(親要件→子。孤児要件を見せる)、リンクつきダッシュボード表。各子の状態は `gate:*` / `loop:*` ラベルと PR の状態から決める。以後の run では同じ計画コメントを編集して更新する(冪等)。
