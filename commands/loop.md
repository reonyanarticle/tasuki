...
**この経路でも、2a を通過した時点で `gate:start-passed` を付ける**(このラベルは「着手してよいと判断済み」を表し、§2(子ごとのゲート実行)の再入表がそれを前提にする。付けないと、`start` を外した契約ではループ由来の子に一度もラベルが付かず、再入のたびに新規 worker が起動する)。
