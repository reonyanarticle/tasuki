...
親 issue に `gate:split-passed` が無ければ判定する。
...
**実行条件**:成果ゲートが PASS し、checks-ci が全 job 成功していること。
