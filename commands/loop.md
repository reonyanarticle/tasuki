...
**旧い契約への後方互換**:契約に `split_criteria` や欄コメントが無い(これらを設ける前の loop-init で作られた)場合は、plugin 側 `profiles/<プロファイル名>.yaml` の同名キーと欄コメントを既定として補って委譲し、`/tasuki:loop-init` の再実行を1度だけ案内する(**そのプロファイルの雛形が plugin 側に無い場合**は廃止されたプロファイルであり、`profiles/development.yaml` を既定に使って続行する)(補いはこの run の中だけで、契約ファイルは書き換えない。黙って基準なしで回すと分割と成果の検査が無警告で弱まる)。
**契約と plugin の版ずれ**:契約の `enabled_gates` にある `checks-*` の provider が、plugin 側の対応 pack の `providers` に存在しない場合は、版ずれとして報告し `/tasuki:loop-init` の再実行を案内して中断する(旧検査を回し続けると、新しい agent 定義が書く成果物と旧検査の要求が食い違い、差し戻しだけが反復して予算を焼く)
...
**契約の `enabled_gates` に `start` が無ければ、この LLM 判定は行わない**(§2a の機械チェックだけで §2c へ進む)。
