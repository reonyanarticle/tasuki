...
1. **観点ごとに独立のレビュー subagent(新規セッション)を起動し、下表の5観点を1観点ずつ回す**(一度に渡すと観点が薄まり、指摘が表層に寄る。maker のコンテキストを渡さない=このレビューも checker 分離)。渡すのは親 PR の全 diff、親 issue の完了の定義、担当観点の定義のみ。導入先に `/code-review` 等のレビューコマンドが導入されていれば、それを観点指定で使ってもよい
