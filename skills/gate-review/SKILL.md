...
完了の定義が「1回のレビューで判断できる範囲」を超えている(定義の項目が多すぎる、互いに独立な価値が同居している)と判断したら、親の分割を促す差し戻しにする(親を2つに割る提案を questions に含める)。
...
- **期待値の根拠(観点 #18)**:各要件の期待値が仕様(受け入れ条件)に対応づいているか。実装出力を写しただけの根拠は「期待値の根拠(仕様由来)の記載なし」として差し戻す
- **生ログと機密の排除(観点 #23)**:本文に載せてよいのは集計値のみ。生ログや生データの貼り付けは TOO_CONCRETE。抜粋に secrets(トークン、鍵、接続文字列)や個人情報が残っていれば差し戻す
