...
### /tasuki:draft の research 実走(tasuki-e2e-research-v2 リポジトリ)

対話式コマンドのため自動 E2E に載せにくかった経路を、起票者役を人間が務める形で実走した。
生の要望(「この前の調査で密度が効くと出たので閾値の測り方を知りたい。arXiv で探せば出ると思う」)を入力にした。

| 検証項目 | 結果 |
|---|---|
| 契約から質問を導く(research の必須欄) | 達成。問い、使い道、価値、予算(timebox)、完了の定義をまとめて1回で質問した |
| 解き方の先取りの分離 | 達成。情報源の示唆(検索先の指定)を本文から外し、参考メモとして起票直後のコメントへ落とした |
| 背景の裏取り | 達成。前提が default branch に無いことを検出したうえで統合ブランチまで辿り、前提が事実であることと、前回調査が判定不能として残した論点であること(二番煎じでないこと)まで確認した |
| 未マージの成果を前提にする是非 | 達成。起票者の問いかけに対し、統合ゲート通過後のマージ待ちが正常状態であることを背景に明記した |
| 事前審査を経た issue が本物の受理ゲートを通るか | 達成。**受理ゲート(opus)が一発で PASS / high**(起票支援を経たことは判定者に伝えていない) |

実走で発見して修正した欠陥: 生成した本文に絶対日付が入った(issue は完走まで参照され続ける文書であり、本文の日付は読むたびに古くなる。相対表現の規則を手順に追加)。
実走で判明した設計上の弱点: 事前審査は自己照合で割れなければ独立判定を経ない。書いた本人が同じ目盛りで照合するため独立判定より弱く、その旨を起票者に伝える規則を追加した(独立判定を望む場合は割れていなくても掛けられる)。
