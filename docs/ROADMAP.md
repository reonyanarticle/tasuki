...
| 書き方の不足はレポートのみ再出力 | 達成(悪いレポートを仕込んだ実走で検証。G3 門前払いが欠落4欄を LLM なしで検出し、worker が実装に触れずレポートのみ再出力、1f 再判定で PASS) |
| G3 差し戻し状態からの run 境界の再入 | 達成(差し戻し直後のセッション死亡を模擬。次の run が起票者待ちにせず、1a と G2 をやり直さず、再出力 worker を自力起動して 1f から復旧) |
...
実ループ未発火のまま残る経路は、`return_to: implementation`(内容の不足による実装差し戻し)と 1g の CI 失敗分岐の2つ(手順・fixture・テストでの検証のみ)。
...
| 依存の循環をエラーとして停止 | fixture・手順検証のみ(**実走未発火**。循環する分割が自然発生しなかったため) |
| レイヤー構成と前進規則 | 達成(L1={#13} → 合流で停止 → 人間マージ → L2={#14}。マージ済み子を除いた再構成も動作)。ただし1レイヤー複数子の同時並行は**未観測**(分割が各層1子だったため) |
