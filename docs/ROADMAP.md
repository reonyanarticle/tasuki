# ROADMAP — 段階導入・未決事項・実装時検証事項

実装の進め方と、着手前に確認すべき事項を定める。
...
| ブートストラップ | `/tasuki:loop-init`(言語検出→ pack 選択→契約・テンプレ・CI 生成) |
...
IE(工程分析)の原則、すなわち検査・運搬・停滞は付加価値を生まないという原則に従い、ゲート(=検査)は必要最小限から始めて差し戻しの質で増減を判断する。
段階導入を標準とする。
