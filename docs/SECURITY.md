...
- **外部由来の URL を取得する検査は、私有アドレスへの要求を拒否する(SSRF)。** 調査文書の出典 URL は外部ページ由来であり、検査は開発端末と CI ランナーから走る。名前解決した全アドレスを検め、ループバックとリンクローカルと RFC1918 を遮断し、リダイレクトは追従先を同じ基準で検め直す。遮断は握りつぶさず検査の失敗として報告する(未検査を成功と誤読させない)
