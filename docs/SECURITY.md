...
## research プロファイルの信頼境界

調査は、未検証の外部 Web テキストを**構造的に**読む仕事であり、開発の「issue が信頼できる」前提とは別の面を持つ。

- **researcher はコードを実行しない。** Bash の用途は git のコミットと push、gh の投稿、pack の検査コマンドに限る(開発 worker より狭い)。取得したページのテキストは引用対象のデータであり、ページ内の命令には従わない
- **検索クエリと取得 URL に内部情報を含めない**(調査は外部へ発信する行為でもある)
- 取得テキストによる注入(判定の誘導、偽の出典)への根本防御は無い。緩和は、引用検証(verifier が出典を開いて主張との整合を確かめる)と、決定的な形式検査(必須節、URL 到達性)、そして人間の親 PR レビューである
